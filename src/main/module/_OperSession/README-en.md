...
| _isNullError | When the inserted value cannot be obtained, whether an error is reported, true: throw an error and exit, false: try to replace it with the default value | No | true | Bool | 1.0=2024.07.10 |
| _defaultValue | The default replacement value when the insertion value cannot be obtained | No | | Object | 1.0=2024.07.10 |
| _setting | Custom data to be inserted into the session | Yes | | Object | 2.0=2026.03.10 |

Supplementary description of module parameters:

### > _isNullError

- When the inserted value cannot be obtained, whether an error is reported, true: throw an error and exit, false: try to replace it with the default value
...
- In the setting, key is the key inserted into session, and value is the corresponding value
...
- Please plan the keys of session data carefully to avoid confusion in a cross-application environment (shared Session)
...
- Be careful when deleting Session values to avoid confusion in a cross-application environment (shared Session)

# ◎ Configuration Notes

The module configuration is set in `_OperSession/Xmas.Config`. When the application is run through Christmas, it will be automatically spliced to `config/application.properties`
...
- However, if a `_ConfigSession` module is present, all Session-related settings should be centralized there; otherwise, configuration conflicts may occur (such as settings overriding each other)
...
```
python3 Christmas/Christmas.py ShellExcute/Run#Module _OperSession
```

You can also run ShellExcute>>Run#Module through the Christmas plug-in, and enter `_OperSession` in the terminal opened by the plug-in.
