...
| _setting | 将要插入session的自定义数据 | 是 |  | Object | 2.0=2026.03.10 |
...
- 如果想要获取的值在passParam的里层（嵌套json），则请以`>>`定位，如`get##key_1>>key_2`
- 删除session值需要慎重，避免在跨应用环境（共享session）引起混乱

# ◎ 配置说明

模块配置在`_OperSession/Xmas.Config`中设置，通过Christmas运行应用时，会自动拼接到`config/application.properties`
...
也可以通过`Christmas插件`运行`ShellExcute>>Run#Module`，并在插件打开的终端中输入`_OperSession`
