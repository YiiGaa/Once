...
public class _ServeDao extends Link {
    ...
    ERRORCODE TXNStart(ObjectNode moduleParam, ObjectNode passParam, ERRORCODE result){
        ...
        try{
            ...
            LOGGER.ERROR("Module-_ServeDao TXNStart exception:"+ TOOLS.GetExceptionInfo(e), moduleParam, passParam);
    ...
    ERRORCODE TXNEnd(ObjectNode moduleParam, ObjectNode passParam, ERRORCODE result){
        ...
        try{
            ...
            LOGGER.ERROR("Module-_ServeDao TXNEnd exception:"+ TOOLS.GetExceptionInfo(e), moduleParam, passParam);
    ...
    ERRORCODE SQLExecute(ObjectNode moduleParam, ObjectNode passParam, ERRORCODE result){
        //STEP::Check last step result
        if(ERRORCODE.IsError(result)){
            return result;
        }
        ...
        try {
            ...
            //STEP::Execute SQL
            JsonNode resultExecute;
            Object ret;
            switch (_inner_SQLType) {
                case "update":
                    LOGGER.INFO("Module-_ServeDao SQLExecute update, sql:" + _inner_SQLString);
                    ret = _ServeDaoConfig.daoHandler.update(_inner_SQLString);
                    if(_inner_isCheckUpdateAffect){
                        if ((Integer)ret<=0){
                            return ERRORCODE.ERR_Module__ServeDao_SQLExecute_Update_Fail;
                        }
                    }
                    resultExecute = TOOLS.JsonInit((Integer)ret);
                    break;
                case "select string":
                    LOGGER.INFO("Module-_ServeDao SQLExecute select string, sql:" + _inner_SQLString);
                    ret = _ServeDaoConfig.daoHandler.queryForObject(_inner_SQLString, String.class);
                    if(_inner_isCheckSelectNotNullError && ret!=null){
                        return ERRORCODE.ERR_Module__ServeDao_SQLExecute_Select_NotNull_Error;
                    } else if(_inner_isCheckSelectNullError && ret==null){
                        return ERRORCODE.ERR_Module__ServeDao_SQLExecute_Select_Null_Error;
                    }
                    resultExecute = TOOLS.JsonInit((String)ret);
                    break;
                case "select int":
                    LOGGER.INFO("Module-_ServeDao SQLExecute select int, sql:" + _inner_SQLString);
                    ret = _ServeDaoConfig.daoHandler.queryForObject(_inner_SQLString, Integer.class);
                    if(_inner_isCheckSelectNotNullError && ret!=null && ((Integer)ret)>0){
                        return ERRORCODE.ERR_Module__ServeDao_SQLExecute_Select_NotNull_Error;
                    } else if(_inner_isCheckSelectNullError && (ret==null || ((Integer)ret)<=0)){
                        return ERRORCODE.ERR_Module__ServeDao_SQLExecute_Select_Null_Error;
                    }
                    resultExecute = TOOLS.JsonInit((Integer)ret);
                    break;
                case "select list":
                    {
                        ...
                        ArrayNode resultArray = TOOLS.JsonInitArray();
                        if (resultList.size() > 0 && _inner_isCheckSelectNotNullError) {
                            return ERRORCODE.ERR_Module__ServeDao_SQLExecute_Select_NotNull_Error;
                        } else if (resultList.size() > 0) {
                            for (Map<String, Object> item : resultList) {
                                ObjectNode jsonObject = TOOLS.JsonInitObject();
                ...
                case "select object":
                    {
                        ...
                        List<Map<String, Object>> resultList = _ServeDaoConfig.daoHandler.queryForList(_inner_SQLString);
                        ObjectNode resultObject = TOOLS.JsonInitObject();
                        if (resultList.size() > 0 && _inner_isCheckSelectNotNullError) {
                            return ERRORCODE.ERR_Module__ServeDao_SQLExecute_Select_NotNull_Error;
                        } else if (resultList.size() > 0) {
                            resultObject = TOOLS.JsonInitObject();
                            for (Map.Entry<String, Object> entry : resultList.get(0).entrySet()) {
                                resultObject.put(entry.getKey(), entry.getValue().toString());
                            }
                        } else if (_inner_isCheckSelectNullError){
                            return ERRORCODE.ERR_Module__ServeDao_SQLExecute_Select_Null_Error;
                        }
            ...
            if(!_inner_resultKey.equals("")) {
                ((ObjectNode)passParam).set(_inner_resultKey, (JsonNode)resultExecute);
            } else if(_inner_resultKey.equals("") && resultExecute instanceof ObjectNode) {
                ((ObjectNode)passParam).setAll((ObjectNode)resultExecute);
            }

        }catch (Exception e){
            LOGGER.ERROR("Module-_ServeDao SQLExecute exception:"+ TOOLS.GetExceptionInfo(e), moduleParam, passParam);
    ...
    ERRORCODE SQLStringDeal(ObjectNode moduleParam, ObjectNode passParam, ERRORCODE result){
        //STEP::Check last step result
        if(ERRORCODE.IsError(result)){
            return result;
        }
        ...
        while (matcher.find()) {
            ...
            if(!returnValue.isMissingNode()){
                String targetValue = "";
                if(returnValue.isArray()){
                    for(JsonNode item : (ArrayNode)returnValue){
                        targetValue += "'"+Tools_SQLPieceEscape(TOOLS.JsonToString(item))+"',";
                    }
                    if (targetValue.endsWith(","))
                        targetValue = targetValue.substring(0, targetValue.length() - 1);
                }else{
                    targetValue = "'"+Tools_SQLPieceEscape(TOOLS.JsonToString(returnValue))+"'";
                }
                ...
                if(returnValue.isArray()){
                    for(JsonNode item : (ArrayNode)returnValue){
                        if(_isSQLReplaceStrict){
                            String testStr = TOOLS.JsonToString(item);
                        ...
                        targetValue += TOOLS.JsonToString(item)+",";
                    }
                    if (targetValue.endsWith(","))
                        targetValue = targetValue.substring(0, targetValue.length() - 1);
                }else{
                    targetValue = TOOLS.JsonToString(returnValue);
                    if(_isSQLReplaceStrict){
    ...
    ERRORCODE SQLUpdate(ObjectNode moduleParam, ObjectNode passParam, ERRORCODE result){
        //STEP::Check last step result
        if(ERRORCODE.IsError(result)){
            return result;
        }
    ...
    ERRORCODE SQLSelect(ObjectNode moduleParam, ObjectNode passParam, ERRORCODE result){
        //STEP::Check last step result
        if(ERRORCODE.IsError(result)){
            return result;
        }
    ...
    ERRORCODE TableCheck(ObjectNode moduleParam, ObjectNode passParam, ERRORCODE result){
        //STEP::Check last step result
        if(ERRORCODE.IsError(result)){
            return result;
        }
        ...
        JsonNode tableObject = (JsonNode)_ServeDaoConfig.tableViewMap.get(_table);
        ...
        } else if(!tableObject.has("type") ||
                  !tableObject.has("keyList") ||
                  !tableObject.has("column")){
        ...
        if(!_inner_tableType.equals("all") &&
           !tableObject.get("type").asString().equals(_inner_tableType)){
                LOGGER.DEBUG("Module-_ServeDao TableCheck table type is illegal, table:" + _table+", type:"+tableObject.get("type").asString());
    ...
    ERRORCODE Insert(ObjectNode moduleParam, ObjectNode passParam, ERRORCODE result){
        //STEP::Check last step result
        if(ERRORCODE.IsError(result)){
            return result;
        }
        ...
        //STEP::Get setting
        String _table = TOOLS.ReadParam("_table", "", moduleParam, passParam);
        JsonNode _target = TOOLS.ReadParam("_target", passParam, moduleParam, passParam);
        ...
        LOGGER.DEBUG("Module-_ServeDao Insert param, _isReplace:" + _target);
        ...
        ObjectNode tableObject = (ObjectNode)_ServeDaoConfig.tableViewMap.get(_table);
        ObjectNode columnMap = (ObjectNode)tableObject.get("column");
        String insertColumn = "";
        String insertValue = "";
        Set<Entry<String,JsonNode>> fields = ((ObjectNode)columnMap).properties();
        for (Entry<String,JsonNode> entry : fields) {
            String key = entry.getKey();
            ObjectNode columnsInfo = (ObjectNode)entry.getValue();
            Boolean isNullAble = (Boolean) columnsInfo.get("isNullAble").asBoolean();
            String type = (String)columnsInfo.get("type").asString();
            Long textMaxLength = (Long) columnsInfo.get("textMaxLength").asLong();

            //STEP-IN::Check whether it lack necessary column
            if(!_target.has(key) && !isNullAble){
            ...
            String value = Tools_SQLPieceEscape(TOOLS.JsonToString(_target.get(key)));
    ...
    ERRORCODE InsertBatch(ObjectNode moduleParam, ObjectNode passParam, ERRORCODE result){
        //STEP::Check last step result
        if(ERRORCODE.IsError(result)){
            return result;
        }
        ...
        //STEP::Get setting
        String _table = TOOLS.ReadParam("_table", "", moduleParam, passParam);
        ArrayNode _target = TOOLS.ReadParam("_target", TOOLS.JsonInitArray(), moduleParam, passParam);
        Boolean _isReplace = TOOLS.ReadParam("_isReplace", false, moduleParam, passParam);
        String _resultKey = TOOLS.ReadParam("_resultKey", "", moduleParam, passParam);
        Boolean _isCheckAffect = TOOLS.ReadParam("_isCheckAffect", true, moduleParam, passParam);
        LOGGER.DEBUG("Module-_ServeDao InsertBatch param, _table:" + _table);
        LOGGER.DEBUG("Module-_ServeDao InsertBatch param, _target:" + _target);
        ...
        for (Entry<String,JsonNode> entry : fields) {
            ...
            int index = 0;
            for(JsonNode item:_target){
                if(item.isObject()){
                    String valueListPiece = insertValueList.get(index);
                    //WHEN-IN::Contains key
                    if(((ObjectNode)item).has(key)){
                        String valuePiece = Tools_SQLPieceEscape(TOOLS.JsonToString(((ObjectNode)item).get(key)));
    ...
    ERRORCODE Delete(ObjectNode moduleParam, ObjectNode passParam, ERRORCODE result){
        //STEP::Check last step result
        if(ERRORCODE.IsError(result)){
            return result;
        }
        ...
        //STEP::Get setting
        String _table = TOOLS.ReadParam("_table", "", moduleParam, passParam);
        ObjectNode _target = TOOLS.ReadParam("_target", passParam, moduleParam, passParam);
        String _filter = TOOLS.ReadParam("_filter", "", moduleParam, passParam);
        String _resultKey = TOOLS.ReadParam("_resultKey", "", moduleParam, passParam);
        Boolean _isCheckAffect = TOOLS.ReadParam("_isCheckAffect", true, moduleParam, passParam);
        LOGGER.DEBUG("Module-_ServeDao Delete param, _table:" + _table);
        LOGGER.DEBUG("Module-_ServeDao Delete param, _target:" + _target);
        ...
        if(!_filter.equals("")){
            ...
            result = SQLStringDeal(moduleParam, passParam, result);
            if(ERRORCODE.IsError(result))return result;
            whereSetting = TOOLS.JsonToString(moduleParam.get("_inner_SQLString"));
        }
        //WHEN::Need to generate by map
            ...
            for (Entry<String,JsonNode> entry : fields) {
                String key = entry.getKey();
                JsonNode columnsInfo = (JsonNode) columnMap.get(key);
                String type = columnsInfo.get("type").asString();
                Long textMaxLength = columnsInfo.get("textMaxLength").asLong();
                ...
                if (_target.get(key).isObject()) {
                    String value = "";
                    for (JsonNode item : _target.get(key)) {
                        String valuePiece = Tools_SQLPieceEscape(TOOLS.JsonToString(item));
                        value += "'" + valuePiece + "',";
                    }
                    if (value.equals("")) {
                        continue;
                    ...
                    String valuePiece = Tools_SQLPieceEscape(TOOLS.JsonToString(_target.get(key)));
    ...
    ERRORCODE Update(ObjectNode moduleParam, ObjectNode passParam, ERRORCODE result){
        //STEP::Check last step result
        if(ERRORCODE.IsError(result)){
            return result;
        }
        ...
        LOGGER.DEBUG("Module-_ServeDao Update param, _target:" + _target);
        ...
        if(!_update.equals("")){
            ...
            result = SQLStringDeal(moduleParam, passParam, result);
            if(ERRORCODE.IsError(result))return result;
            updateSetting = moduleParam.get("_inner_SQLString").asString();
        }
        //WHEN::_filter is set(replace @xxx@ by SQLStringDeal())
        if(!_filter.equals("")){
            whereSetting = _filter;
            ...
            result = SQLStringDeal(moduleParam, passParam, result);
            if(ERRORCODE.IsError(result))return result;
            whereSetting = moduleParam.get("_inner_SQLString").asString();
        }
        //WHEN::Need to generate by map
        ObjectNode tableObject = (ObjectNode)_ServeDaoConfig.tableViewMap.get(_table);
        ObjectNode columnMap = (ObjectNode)tableObject.get("column");
        if(_filter.equals("")||_update.equals("")) {
            ...
            for (Entry<String,JsonNode> entry : fields) {
                String key = entry.getKey();
                JsonNode columnsInfo = columnMap.get(key);
                Boolean isNullAble = (Boolean) columnsInfo.get("isNullAble").asBoolean();
                String type = (String)columnsInfo.get("type").asString();
                Long textMaxLength = (Long) columnsInfo.get("textMaxLength").asLong();
                Boolean isKey = (Boolean) columnsInfo.get("isKey").asBoolean();

                //WHEN-IN::Continue when lack column
                if (!_target.has(key)) {
                    continue;
                }
                JsonNode targetValue = _target.get(key);

                //STEP-IN::Stitching updateSetting(when not key column and not JSONArray)
                if(isNeedGenerateUpdate && !isKey && !targetValue.isArray()){
                    String valuePiece = Tools_SQLPieceEscape(TOOLS.JsonToString(targetValue));
                    if(type.equals("text") && valuePiece.length()>textMaxLength){
    ...
    ERRORCODE Select(ObjectNode moduleParam, ObjectNode passParam, ERRORCODE result){
        //STEP::Check last step result
        if(ERRORCODE.IsError(result)){
            return result;
        }
        ...
        LOGGER.DEBUG("Module-_ServeDao Select param, _target:" + _target);
        ...
        if(!_get.equals("")){
            ...
            result = SQLStringDeal(moduleParam, passParam, result);
            if(ERRORCODE.IsError(result))return result;
            getSetting = (String) moduleParam.get("_inner_SQLString").asString();
        }
        //WHEN::get all column
        else{
            getSetting = "*";
        ...
        //WHEN::Need to generate by map
            ...
            JsonNode tableObject = (JsonNode)_ServeDaoConfig.tableViewMap.get(_table);
            JsonNode columnMap = (JsonNode)tableObject.get("column");
            Set<Entry<String,JsonNode>> fields = ((ObjectNode)columnMap).properties();
            for (Entry<String,JsonNode> entry : fields) {
                String key = entry.getKey();
                JsonNode columnsInfo = columnMap.get(key);
                Boolean isNullAble = columnsInfo.get("isNullAble").asBoolean();
                String type = columnsInfo.get("type").asString();
                Long textMaxLength = columnsInfo.get("textMaxLength").asLong();
                Boolean isKey = columnsInfo.get("isKey").isBoolean();
                ...
                if(targetValue.isArray()){
                    String value = "";
                    for (JsonNode item : targetValue) {
                        String itemValue = Tools_SQLPieceEscape(TOOLS.JsonToString(item));
                        value = "'" + itemValue + "',";
                    }
                    if (value.equals("")) {
                        continue;
                    ...
                    String value = Tools_SQLPieceEscape(TOOLS.JsonToString(targetValue));
    ...
    ERRORCODE SelectCount(ObjectNode moduleParam, ObjectNode passParam, ERRORCODE result){
        //STEP::Check last step result
        if(ERRORCODE.IsError(result)){
            return result;
        }
        ...
        result = SQLExecute(moduleParam, passParam, result);

        return result;
    }

    ERRORCODE SelectList(ObjectNode moduleParam, ObjectNode passParam, ERRORCODE result){
        //STEP::Check last step result
        if(ERRORCODE.IsError(result)){
            return result;
        }
        ...
        if(!_orderBy.equals("")){
            ...
            result = SQLStringDeal(moduleParam, passParam, result);
            if(ERRORCODE.IsError(result))return result;
            orderByStr = moduleParam.get("_inner_SQLString").asString();
        }
    ...
    ERRORCODE SelectObject(ObjectNode moduleParam, ObjectNode passParam, ERRORCODE result){
        //STEP::Check last step result
        if(ERRORCODE.IsError(result)){
            return result;
        }
        ...
        try{
            ...
            switch (_action){
                case "insert":
                    ...
                    result = TableCheck(moduleParam, passParam, result);
                    result = Insert(moduleParam, passParam, result);
                    result = SQLExecute(moduleParam, passParam, result);
                    break;
                case "insert batch":
                    moduleParam.put("_inner_tableType", "table");
                    result = TableCheck(moduleParam, passParam, result);
                    result = InsertBatch(moduleParam, passParam, result);
                    result = SQLExecute(moduleParam, passParam, result);
                    break;
                case "delete":
                    moduleParam.put("_inner_tableType", "table");
                ...
                case "update":
                    ...
                    result = TableCheck(moduleParam, passParam, result);
                    //SQLStringDeal() is called in Update()
                    result = Update(moduleParam, passParam, result);
                    result = SQLExecute(moduleParam, passParam, result);
                    break;
                case "select object":
                    ...
                    result = Select(moduleParam, passParam, result);
                    result = SelectObject(moduleParam, passParam, result);
                    result = SQLExecute(moduleParam, passParam, result);
                    break;
                case "select list":
                    moduleParam.put("_inner_tableType", "all");
                    ...
                    //SQLStringDeal() is called in Select()
                    result = Select(moduleParam, passParam, result);
                    result = SelectCount(moduleParam, passParam, result);
                    result = SelectList(moduleParam, passParam, result);
                    //SQLExecute() is called in SelectCount()/SelectList()
                    break;
                case "sql update":
                    result = SQLUpdate(moduleParam, passParam, result);
                    result = SQLStringDeal(moduleParam, passParam, result);
                    result = SQLExecute(moduleParam, passParam, result);
                    break;
                case "sql select":
                    result = SQLSelect(moduleParam, passParam, result);
                    result = SQLStringDeal(moduleParam, passParam, result);
                    result = SQLExecute(moduleParam, passParam, result);
                    break;
                case "txn start":
                    result = TXNStart(moduleParam, passParam, result);
                    break;
                case "txn end":
                    result = TXNEnd(moduleParam, passParam, result);
                    break;
                default:
        ...
        try{
            if(transactionIsAutoErrorRollBack.get()!=null){
                ...
                if(position.equals(transactionPosition.get())){
                    ObjectNode passParam = (ObjectNode) dataPool.get("passParam");
                    ObjectNode moduleParam = (ObjectNode) dataPool.get("moduleParam");
                    ...
                    TXNEnd(moduleParam, passParam, result);
                }
            }
        }catch (Exception e){
