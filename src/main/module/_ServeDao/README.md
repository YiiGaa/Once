...
- 设置MySQL数据库连接信息，在`_ServeDao/Xmas.Config`文件中进行设置
...
| _isSQLReplaceStrict | 动态替换SQL碎片数据时，是否以严格方式替换，true:严格替换，false:不检查字符 | 否       | true                                                    | Bool   | 1.0=2024.06.20 |
| _resultKey          | 记录结果的Key                                                | 是       | 将查询结果按字段直接插入数据池（passParam） | String | 1.0=2024.06.20 |
...
- 记录结果的Key
- 默认情况下，或者将`_resultKey`设置为空字符串`""`，会将查询结果按字段直接插入数据池（passParam）
- 查询结果被限制在第1条数据
...
| _resultKey          | 记录结果列表的Key                                            | 是       | result                | String | 1.0=2024.06.20 |
| _isGetNullError     | 获取不到数据时是否报错，true:报错，false:不报错              | 否       | false                 | Bool   | 1.0=2024.06.20 |
| _isGetNotNullError  | 获取到数据时是否报错，true:报错，false:不报错                | 否       | false                 | Bool   | 1.0=2024.06.20 |
...
| _selectType         | 指定查询的结果类型                                           | 是       | select list | String | 1.0=2024.06.20 |
| _isSQLReplaceStrict | 动态替换SQL碎片数据时，是否以严格方式替换，true:严格替换，false:不检查字符 | 否       | true        | Bool   | 1.0=2024.06.20 |
| _resultKey          | 记录结果的Key                                                | 否       | 不记录结果  | String | 1.0=2024.06.20 |
| _isGetNullError     | 获取不到数据时是否报错，true:报错，false:不报错              | 否       | false       | Bool   | 1.0=2024.06.20 |
| _isGetNotNullError  | 获取到数据时是否报错，true:报错，false:不报错                | 否       | false       | Bool   | 1.0=2024.06.20 |
...
- 如果`_selectType`被设置为`select object`，且`_resultKey`省略或设置为空字符串`""`，将查询结果按字段直接插入数据池（passParam）
...
| _isAutoErrorRollBack | 业务逻辑结束时，发生错误是否自动回滚事务，true:发生错误回滚，不发生错误提交，false:不发生错误回滚，发生错误提交 | 是       | true         | Bool   | 1.0=2024.06.20 |
...
| _isCommit | 是否提交事务，true:提交，false:回滚 | Yes | true | Bool | 1.0=2024.06.20 |

# ◎ 配置说明

模块配置在`_ServeDao/Xmas.Config`中设置，通过Christmas运行应用时，会自动拼接到`config/application.properties`
...
- 自动获取表字段，需要在`_ServeDao/Xmas.Config`文件中设置
...
- 创建完测试用的数据库、数据表后，修改`_ServeDao/Xmas.Config`文件中的关于数据库的配置
...
也可以通过`Christmas插件`运行`ShellExcute>>Run#Module`，并在插件打开的终端中输入`_ServeDao`
