...
- select list-query a list of records
...
- Set the MySQL database connection information and set it in the `_ServeDao/Xmas.Config` file
- If automatic table information retrieval is enabled (enabled by default), the database must be MySQL 8
...
| _target        | Target data                                                  | No        | Uses passParam as the target data | Object | 1.0=2024.06.20 |
...
- There are three cases in which a table field can be empty: the field is nullable, the field has a default value, or the field is auto-increment
...
- For details about strict replacement, please refer to `Special Notes-Dynamic Replacement Data`
...
| _filter             | SQL fragment for filtering (WHERE part of UPDATE SQL)        | No        | Automatically generated    | String | 1.0=2024.06.20 |
| _isSQLReplaceStrict | When dynamically replacing SQL fragmented data, whether to replace it in a strict way, true: strictly replace, false: do not check the characters | No        | true                       | Bool   | 1.0=2024.06.20 |
| _resultKey          | Key to record the results                                    | No        | Do not record the results  | String | 1.0=2024.06.20 |
| _isCheckAffect      | Whether to check the execution result, true: check (the number of updated rows <=0 error), false: do not check | No        | true                       | Bool   | 1.0=2024.06.20 |
...
- Add version: 1.0=2024.06.20
- It is required to turn on the function of automatically obtaining table fields, otherwise this function is invalid
- The query result is limited to the first row
...
| _isSQLReplaceStrict | When dynamically replacing SQL fragmented data, whether to replace it in a strict way, true: strictly replace, false: do not check the characters | No        | true                                                         | Bool   | 1.0=2024.06.20 |
| _resultKey          | Key to record the results                                    | Yes       | Insert the query results directly into the data pool by field (passParam) | String | 1.0=2024.06.20 |
...
- For cross-table or cross-database queries, it is recommended to use views to reduce SQL complexity
...
- By default, or set `_resultKey` to an empty string `""`, the query results will be inserted directly into the data pool (passParam) by field
...
| _resultKey          | Key to record the results                                    | Yes       | result                     | String | 1.0=2024.06.20 |
| _isGetNullError     | Whether to report an error when the data cannot be obtained, true: error, false: no error | No        | false                      | Bool   | 1.0=2024.06.20 |
| _isGetNotNullError  | Whether to report an error when obtaining the data, true: error, false: no error | No        | false                      | Bool   | 1.0=2024.06.20 |
...
```yaml
>_ServeDao -sql update
    _sql: UPDATE table SET key = '@key@' WHERE id = '@id@'
    _resultKey: result
    _isCheckAffect: true
```
...
>_ServeDao -sql select
    _sql: SELECT * FROM table WHERE key = '@key@'
...
| _sql                | SQL statements for query                                     | Yes       |                           | String | 1.0=2024.06.20 |
| _selectType         | Specify the result type of the query                         | Yes       | select list               | String | 1.0=2024.06.20 |
| _isSQLReplaceStrict | When dynamically replacing SQL fragmented data, whether to replace it in a strict way, true: strictly replace, false: do not check the characters | No        | true                      | Bool   | 1.0=2024.06.20 |
| _resultKey          | Key to record the results                                    | No        | Do not record the results | String | 1.0=2024.06.20 |
| _isGetNullError     | Whether to report an error when the data cannot be obtained, true: error, false: no error | No        | false                     | Bool   | 1.0=2024.06.20 |
| _isGetNotNullError  | Whether to report an error when obtaining the data, true: error, false: no error | No        | false                     | Bool   | 1.0=2024.06.20 |
...
- If `_selectType` is set to `select object`, and `_resultKey` is omitted or set to an empty string `""`, insert the query results directly into the data pool by field (passParam)
...
| _isAutoErrorRollBack | At the end of the business logic, whether the error automatically rolls back the transaction, true: error rollback, no error submission, false: no error rollback, error submission | Yes       | true                    | Bool   | 1.0=2024.06.20 |
...
| _isCommit | Whether to submit the transaction, true: submit, false: rollback | Yes       | true    | Bool   | 1.0=2024.06.20 |

# ◎ Configuration Notes

The module configuration is set in `_ServeDao/Xmas.Config`. When the application is run through Christmas, it will be automatically spliced to `config/application.properties`
...
- If the value you want to get is in the inner layer of passParam (nested json), please locate it with `>>`, such as `@key_1>>key_2@`

- If the replacement target data is a `Json Array`, it will be automatically expanded, such as `list1, list2, list3`

- The part marked with `'@xxx@'` will be replaced directly with the obtained data, such as `'data value...'`. To prevent SQL injection, `'` in the obtained data will be escaped as `''`
...
- Automatically get table fields, which needs to be set in the `_ServeDao/Xmas.Config` file
...
- After creating the database and data table for testing, modify the configuration of the database in the `_ServeDao/Xmas.Config` file.
...
```
python3 Christmas/Christmas.py ShellExcute/Run#Module _ServeDao
```

You can also run ShellExcute>>Run#Module through the Christmas plug-in, and enter `_ServeDao` in the terminal opened by the plug-in
