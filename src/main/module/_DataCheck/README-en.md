...
| -------- | ------------------------------------------------------------ | --------- | ------- | ------ | -------------- |
| _isClean | Whether to clean redundant fields, false: not cleaned, true: cleaned | Yes       | false   | Bool   | 1.0=2024.06.15 |
...
- Validation template for passParam, type Json Object

- `2.0=2026.03.10` added, if the `_param` value is set to `{}` and `_isClean` is set to true, it indicates clearing passParam
...
- The value of the template Json can use prefixes such as `int##`, `str##`, `double##`, and `long##` to check the value type. `int##` means integer, `str##` means string (default), `double##` means floating-point number, `long##` means long integer, and `bool##` means boolean (`true`/`false`). For example, `double##3.5/8.3` means the value must be 3.5 or 8.3
...
- The value of the template Json can be `[]`, and the value range can be set internally, such as `["1","2",{"key":"value"}]`. As long as it matches one configured element, the check will pass. If a value range is configured, the Json array cannot be empty
...
You can also run ShellExcute>>Run#Module through the Christmas plug-in, and enter `_DataCheck` in the terminal opened by the plug-in
...
- Adapt to Once3
- New `bool##` type limit, `int##` is no longer compatible with bool type
- Added, if the _param value is set to `{}`, and _isClean is set to `true`, it indicates that passParam is to be cleared
