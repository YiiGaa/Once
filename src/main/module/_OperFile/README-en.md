...
- Set the parent directory of the file operation, `module._OperFile.rootPath` in file `_OperFile/Xmas.Config`, and ensure that the program has permission to operate this directory
...
- It is only suitable for uploading small files. For large files, please consider other solutions such as chunked upload
...
| _targetKey             | The key corresponding to the upload file in the request form | Yes       | file                           | String | 1.0=2024.07.02 |
...
| _resultPathKey         | The key to put the result (uploaded file path)               | Yes       | Use the setting of `targetKey` | String | 1.0=2024.07.02 |
...
- If multiple files are recorded with the same key in the form, the module will automatically upload them together
...
- This setting only takes effect when multiple files are recorded with the same key in the request form and the value is `> 0`
...
- Priority of limits: nginx/tomcat request size limits > spring.servlet.multipart.max-request-size > spring.servlet.multipart.max-file-size > module upload size limits
...
- It is recommended that `_targetFileName` contain `uuid`, and that `uuid` be updated each time the module is called to receive an uploaded file
...
- Add version: 1.0=2024.07.02
- Please consider carefully. After a file is deleted, it is generally not possible to roll it back or recover it.
...
| key                | Description                                                  | Necessary | Default | Type   | Add/Update     |
| ------------------ | ------------------------------------------------------------ | --------- | ------- | ------ | -------------- |
| _action            | Specify the module action and fix it as "move"              | Yes       |         | String | 1.0=2024.07.02 |
...
- If the move is performed synchronously, the module will return the move result. If it is performed asynchronously, you can confirm success only through the logs

# ◎ Configuration Notes

The module configuration is set in `_OperFile/Xmas.Config`. When the application is run through Christmas, it will be automatically spliced to `config/application.properties`

| key                                       | Description                                                  | Necessary | Default     | Type   | Add/Update     |
| ----------------------------------------- | ------------------------------------------------------------ | --------- | ----------- | ------ | -------------- |
| spring.servlet.multipart.max-file-size    | Limit single file size in advance through the servlet, e.g. 1MB | No        | 1MB         | String | SpringBoot |
...
- Limit single file size in advance through the servlet, e.g. 1MB
...
Modify `module._OperFile.rootPath` in `_OperFile/Xmas.Config` before testing (it specifies the parent directory for uploaded files)

> The directory pointed to by `module._OperFile.rootPath` must have read and write permissions

Then, you need to run the module separately. In the project root directory, run the shell instruction

```
python3 Christmas/Christmas.py ShellExcute/Run#Module _OperFile
```

You can also run ShellExcute>>Run#Module through the Christmas plug-in, and enter `_OperFile` in the terminal opened by the plug-in.
