...
- 设置文件操作的父目录，在`_OperFile/Xmas.Config`文件中的`module._OperFile.rootPath`，并确保程序有操作此目录的权限
...
| _targetKey             | 上传文件在请求表单中对应的key                                | 是       | file                   | String | 1.0=2024.07.02 |
| _isNullError           | 当请求表单中获取不到文件时，是否报错。true:报错，false:不报错 | 否       | true                   | Bool   | 1.0=2024.07.02 |
| _maxCount              | 上传文件的最大个数                                           | 否       | 不限制(-1)             | Int    | 1.0=2024.07.02 |
| _maxFileSize           | 单个文件的最大存储空间限制（单位：字节）                     | 否       | 不限制(-1)             | Long   | 1.0=2024.07.02 |
...
| _resultPathKey         | 存放上传完成的文件路径的key                                  | 是       | 采用`_targetKey`的设置 | String | 1.0=2024.07.02 |
...
- 当请求表单中采用多个相同的key记录多个文件时，且`值>0`，此设置才会生效
- 如果`值<1`，则视为不限制
- 限制存在优先级，nginx、tomcat请求数据的限制 > spring.servlet.multipart.max-request-size > spring.servlet.multipart.max-file-size > 模块上传时的文件大小限制
...
- 如果在非同一文件系统下，迁移文件/文件夹，可能需要一些时间，更推荐采用异步的方式
...
模块配置在`_OperFile/Xmas.Config`中设置，通过Christmas运行应用时，会自动拼接到`config/application.properties`
...
- 限制存在优先级，nginx、tomcat请求数据的限制 > spring.servlet.multipart.max-request-size > spring.servlet.multipart.max-file-size > 模块对上传文件大小的限制
...
测试前先修改`_OperFile/Xmas.Config`文件中的`module._OperFile.rootPath`设置（用于上传文件的父目录）

> `module._OperFile.rootPath`指向的目录必须具有读写权限
...
也可以通过`Christmas插件`运行`ShellExcute>>Run#Module`，并在插件打开的终端中输入`_OperFile`
