# Controller 模块：FillingHashParam

## 模块功能简述

FillingParam用作批量操作（批量添加、批量修改），检查某个字段的对应数组中的每个Json数据中的必要参数与防止多余参数进入，主要功能包括：

> 对passParam数据池中某个字段对应的数组中的每个Json数组参数进行填充。

- 填充参数，包括生成uuid、获取session值、时间、拼接参数等
- 备份当前passParam参数（给每一个参数的key添加前缀）

举个例子，请求参数如下的话，此模块填充的是aa字段对应的数组中的每个Json进行参数填充，如向aa下每个Json数据添加id参数。另外，aa字段是否含有值需先由检查必要参数模块（CheckNecessaryParam）判断。
...
| 模块名           | 最新版本 | 模块位置   | 依赖服务 |
| ---------------- | -------- | ---------- | -------- |
| FillingHashParam | v1.0     | Controller | 无       |
...
- 备份数据，备份数据的key会加上指定前缀（tempCacheBackUp_），一般对应缓存模块（CacheOperation）使用。备份整个passParam数据而非单个字段数组。
...
| ------------------- | ---- | -------------------- | ------ | -------- | ------------------------------------------------------------ |
| module_backUpPrefix | 否   | String               |        |          | 备份当前passParam数据，备份数据的key上加入module_backUpPrefix对应值的前缀，一般对应缓存模块（CacheOperation）使用 |
| 自定义              | 否   | 特殊参数，详见以下说 |        |          | 需要填充到passParam的目标key                                 |
...
| get          | 是                     | passParam数据池中的key                  | 先获取当前Json的key对应的值，如果获取失败，会向passParam获取对应的值 |
| self         | 否                     |                                         | 当前key对应的值                                              |
| index        | 否                     |                                         | 当前Json位于数组的位置，0、1、2、3                           |
| 自定义       | 否                     |                                         | 直接使用此字符串拼接                                         |
